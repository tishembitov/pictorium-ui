...
const LOG_PREFIX = '[RemoveFromAllBoards]';
...
const createClearedPin = (pin: PinResponse): PinResponse => {
  ...
  logPinState('After clear update', updated);
  return updated;
};
...
export const useRemovePinFromAllBoards = (options: UseRemovePinFromAllBoardsOptions = {}) => {
  ...
  const mutation = useMutation({
    ...
    mutationFn: async ({ pinId, shouldDelete }: { pinId: string; shouldDelete: boolean }) => {
      ...
      if (shouldDelete) {
        logDecision('DELETE PIN', 'Decision from pre-mutation');
        await pinApi.delete(pinId);
        return { deleted: true, pinId };
      } else {
        logDecision('REMOVE FROM BOARDS ONLY', 'Decision from pre-mutation');
    ...
    onMutate: async ({ pinId, shouldDelete }) => {
      ...
      if (shouldDelete) {
        ...
        console.log(`${LOG_PREFIX} Optimistic: Clearing board saves only`);
        ...
        // Remove from saved queries if not in profile
        if (previousPin && !previousPin.isSavedToProfile && userId) {
          console.log(`${LOG_PREFIX} Pin not in profile - removing from saved queries`);
          queryClient.getQueriesData<InfiniteData<PagePinResponse>>({ queryKey: queryKeys.pins.all })
            .forEach(([key]) => {
    ...
    onSuccess: (result) => {
      ...
      } else if (showToast) toast.success('Pin removed from all boards');
    
      // ✅ Всегда инвалидируем все доски
