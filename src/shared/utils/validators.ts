...
export const isValidUsername = (username: string): { valid: boolean; error?: string } => {
  ...
  if (username.length < TEXT_LIMITS.USERNAME_MIN) {
    return { valid: false, error: `Username must be at least ${TEXT_LIMITS.USERNAME_MIN} characters` };
  }
  
  if (username.length > TEXT_LIMITS.USERNAME_MAX) {
    return { valid: false, error: `Username must be less than ${TEXT_LIMITS.USERNAME_MAX} characters` };
  }
